...
function buildPrompt(scene, tags) {
  return `Write one Xiaohongshu (RED) style note in Chinese.
- Scene: ${scene}; keep copy focused on this scenario, casual and friendly, not salesy.
- Structure: catchy title (<=20 chars), newline, then body; total <=500 chars.
- Body: include experience details (environment/service/effects), blend 2-5 emojis naturally.
- No numbering, no words like "below" or ad speak.
- End with the exact tag string on a new line: ${tags}.
Return strictly JSON {"title":"...","body":"..."}, nothing else.`;
}
...
export default async function handler(req, res) {
  ...
  const { scene = "general", tags = "#demo #tags" } = req.body || {};
